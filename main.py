...
def addemployee():
    ...
    def submitadd():
        id = idval.get()                      #حصلت على البيانات من جداول الإدخالات
        name = nameval.get()
        ...
        addedtime = time.strftime("%H:%M:%S") #أضافة الوقت
        addeddate = time.strftime("%d/%m/%Y") #أضافة التاريخ
        try:
            ...
            # هنا لمسح استمارة الإدخال بعد إضافة التفاصيل إلى قاعدة البيانات
            if (res == True):
                idval.set('')
                nameval.set('')
                mobileval.set('')
        ...
        for i in datas:
            vv = [i[0], i[1], i[2], i[3], i[4], i[5], i[6], i[7], i[8]] #هنالأضافة و إعداد البيانات كقائمة للطباعة
            employeetable.insert('', END, values=vv)                    #لإظهار البيانات، ومعاينة الأشخاص القابلة للتوظيف، والمطبوعات من البداية إلى النهاية
            
        # نهاية تعريف sumbitbutton 
        
    # أضافة نافذة خاصة لدخول employeedata   
    addroot = Toplevel(master=DataEntryFrame)
    addroot.grab_set()
    addroot.geometry('470x470+220+200')
    addroot.title('Enter details')
    addroot.config(bg='light blue')
    addroot.resizable(False, False) #جعل النافذة غير قابلة للتغيير
    
    # هنا لأضافة الموظفين الجدد
    ...
    addroot.mainloop()
#أنتهيت من أضافة الموظفين الجدد 

#للبحث عن الموظفين   
def searchemployee():
    # تحديد زر البحث في شكل موظف
    def search():
        id = idval.get()
        name = nameval.get()
        mobile = mobileval.get()
        ...
        #للبحث عن السجلات 
        if (id != ''):                                       
            strr = 'select *from employeedata1 where id=%s'    #هنا لاختيار جميع السجلات بالقيمة المدخلة
            mycursor.execute(strr, (id))                       #تنفيذ الأمر
            datas = mycursor.fetchall()                        #أستحضار جميع السجلات في البيانات
            employeetable.delete(*employeetable.get_children())# لحذف وحدة التحكم (showdataframe)
            for i in datas:
                vv = [i[0], i[1], i[2], i[3], i[4], i[5], i[6], i[7], i[8]]
                employeetable.insert('', END, values=vv)  # لتطبع سجلاً تم إحضاره من البداية إلى النهاية لتلك القيمة
        elif (name != ''):
            ...
            for i in datas:
                vv = [i[0], i[1], i[2], i[3], i[4], i[5], i[6], i[7], i[8]]
                employeetable.insert('', END, values=vv)        
    #أنتهيت من البحث عن السجلات 
    
    # إنشاء نافذة من نموذج البحث 
    ...
    searchroot.mainloop()
#أنتهيت من تحديد البحث   

#تعريف بالموظفين المحذوفيين
def deleteemployee():
    cc = employeetable.focus()         #لتركيز على السجل الذي ننقر عليه ونحصل على بيانات
    content = employeetable.item(cc)
    pp = content['values'][0]          #هنا لطلب هوية السجل الخاص للحذف 
...
# أنتهيت من تحديد المحذوفين 

#تحديد المستجدات 
def updateemployee():
    #تحديد updatebutton في استمارة إدخال التحديث 
    def update():
        id = idval.get()
        name = nameval.get()
        mobile = mobileval.get()
    ...
    cc = employeetable.focus()      # سيرجع لنا جميع تفاصيل السجل الحالي عند النقر عليه وملء مدخل التحديث
    content = employeetable.item(cc)
    pp = content['values']
    if (len(pp) != 0):
        idval.set(pp[0])
...
#تحديد قاعدة بيانات لتسجيل الدخول 
def Connectdb():
    
    # تحديد زر الدخول والاتصال بقاعدة البيانات 
    def submitdb():
        global con, mycursor
        host = "localhost"
        user = userval.get()
    ...
    #إنشاء نافذة لزر قاعدة بيانات تسجيل الدخول 
    ...
    dbroot.mainloop()
#أنتهيت من زر الدخول لقاعدة البيانات 

# تحديد الوقت و التاريخ   
def tick():
...
#إنشاء إطارين في النافذة الرئيسية 
...
#انتهيت من عرض البيانات

# الان الجانب العلوي من النافذة الأساسية  
# العنوان
...
#حقيقةً كان من المفترض أن الفكرة مختلفة و نوع الإطار ...لكن لضيق الوقت حذفت المشروع الأساسي  الذي كان بأستخدام (dgango)
#أعذروني عن المشروع حاولت الأنتهاء منه قدر الأمكان بالوقت المحدد 